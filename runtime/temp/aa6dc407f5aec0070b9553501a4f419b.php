<?php if (!defined('THINK_PATH')) exit(); /*a:4:{s:35:"template/shop/blue/Login/email.html";i:1510990152;s:42:"template/shop/blue/Login/registerBase.html";i:1508397552;s:32:"template/shop/blue/urlModel.html";i:1505982578;s:35:"template/shop/blue/loginTopNav.html";i:1508291124;}*/ ?>
<!DOCTYPE html>
...
$(document).ready(function(){
	$("#emailregistermodel-email").change(function(){
		if(!($(this).val().search(/\w+([-+.]\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*/)==-1)){	
			...
			$.ajax({
				type: "get",
				url: "<?php echo __URL('SHOP_MAIN/login/email'); ?>",
				data: {"email":user_email},
				success: function(data){
					if(data){
						$('#emailregistermodel-email').css("border","1px solid red");
						$("#emailyz").css("color","red").text("<?php echo lang('mailbox_has_been_registered'); ?>");
	...
	$("#btn_submit").click(function(){
		...
		<?php endif; if($notice['noticeEmail'] == 1): ?>
			if(code.length != 6){
				...
				$("#qryzm").css("color","red").text("<?php echo lang('mailbox_checksum_error'); ?>");
				$("#repass").css("border","1px solid #ccc");
				$("#qrrepass").css("color","red").text("");
				return false;
			}
			...
			$.ajax({
				type:"post",
				url: "<?php echo __URL('SHOP_MAIN/login/check_code'); ?>",
	...
	$("#sendOutCode").click(function (){
		...
		$.ajax({
			...
			success: function(data){
				if(data){
					$('#emailregistermodel-email').css("border","1px solid red");
					$("#emailyz").css("color","red").text("<?php echo lang('mailbox_number_has_been_registered'); ?>");
					$("#isset_email").attr("value","<?php echo lang('existence'); ?>");
					return false;
				}else{
			 		//判断输入的验证码是否正确 
			 		if(is_click != 0){
					...
					$.ajax({
						type: "POST",
						url: "<?php echo __URL('SHOP_MAIN/login/sendemailcode'); ?>",
						data: {"email":email,"vertification":vertification},
						success: function(data){
							if(data['code'] == 0){
								$("#emailyz").css("color","red").text("<?php echo lang('send_successfully'); ?>");
								time();
								is_click = 0;
							}else{
...
<div class="reg-type">
	...
	<ul class="clearfix">
		...
		<?php endif; ?>
				...
				<div class="reg-wrap">
					<div class="reg-wrap-con" style="background: url(__TEMP__/<?php echo $style; ?>/public/images/blue_login_banner.png) no-repeat right 70px;">
						<form id="EmailRegisterModel" class="form-horizontal" name="EmailRegisterModel" action="<?php echo __URL('SHOP_MAIN/login/email'); ?>" method="post">
							...
							<div class="form-group form-group-spe">
								<label for="emailregistermodel-email" class="input-left">
									...
									<span><?php echo lang('mailbox'); ?>：</span>
								</label>
								<div class="form-control-box">
									<input type="email" id="emailregistermodel-email" class="form-control" name="email" placeholder="<?php echo lang('enter_your_email_address'); ?>" />
							...
							<div class="form-group form-group-spe">
								...
								<div class="form-control-box">
									<input type="password" id="repass" class="form-control" name="qrpassword" autocomplete="off" placeholder="<?php echo lang('confirm_the_password'); ?>">
									<!--  <i class="fa fa-eye-slash pwd-toggle" data-id="password"></i>	-->	
							...
							<div class="form-group form-group-spe">
								<label for="emailregistermodel-email_captcha" class="input-left">
									<span class="spark">*</span>
									<span><?php echo lang('mailbox_check_code'); ?>：</span>
								</label>
								<div class="form-control-box">
									...
									<input type="button" class="phonecode" id="sendOutCode" name="code_email" value="<?php echo lang('get_mailbox_check_code'); ?>" />
									<span id="qryzm"></span>
								</div>
								<div class="invalid"></div>
							</div>
